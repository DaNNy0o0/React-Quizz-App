import React from "react";

import quizCompleted from "../assets/quiz-complete.png";
import QUESTIONS_ARRAY from "../questions.js";

const Summary = ({ userAnswers }) => {

    const skippedAnswers = userAnswers.filter(answer => answer === null)
    const correctAnswers = userAnswers.filter((answer, i) => answer === QUESTIONS_ARRAY[i].answers[0] )

    const skippedAnswersShare = (skippedAnswers.length / userAnswers.length) * 100
    const correctAnswersShare = (correctAnswers.length / userAnswers.length) * 100

    const wrongAnswersShare = 100 - skippedAnswersShare - correctAnswers

  return (
    <div id="summary">
      <img src={quizCompleted} alt="Quiz Completed" />
      <h2>Quiz Completed!</h2>
      <div id="summary-stats">
        <p>
          <span className="number">{skippedAnswersShare.toFixed(0)}%</span>
          <span className="text">skipped</span>
        </p>
        <p>
          <span className="number">{correctAnswersShare.toFixed(0)}%</span>
          <span className="text">answered correctly</span>
        </p>
        <p>
          <span className="number">{wrongAnswersShare.toFixed(0)}%</span>
          <span className="text">answered incorrectly</span>
        </p>
      </div>
      <ol>
        {userAnswers.map((answer, index) => {
          let cssClass = "user-answer";

          if (answer === null) {
            cssClass += ' skipped'
          } else if (answer === QUESTIONS_ARRAY[index].answers[0]){
            cssClass += ' correct'
          } else {
            cssClass += ' wrong'
          }

          return (
            <li key={index}>
              <h3>{index + 1}</h3>
              <p className="question">{QUESTIONS_ARRAY[index].text}</p>
              <p className={cssClass}>{answer ?? "Skipped!"}</p>
            </li>
          );
        })}
      </ol>
    </div>
  );
};

export default Summary;
